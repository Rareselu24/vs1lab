...
async function discoveryHandler(submitEvent) {
    ...
    const latitude = document.getElementById('latitudeSearch').value;
    const longitude = document.getElementById('longitudeSearch').value;
    const query = document.getElementById('search_input').value;

    let url = `/api/geotags?latitude=${latitude}&longitude=${longitude}`;

    if (query != "") {
        url += `&search_input=${encodeURIComponent(query)}`;
    }
